...
def write(state):
    ...
    if uploaded_file:
        ...
        st.markdown("Para continuar, navegue pela aba **lateral esquerda** e selecione as opções.")
        ...
        if not df.empty:
            ...
            if len(state.fltTags)>10:
                expanderFltTags.warning('Foram selecionadas mais de **10 TAGs**')
            ...
            selStart = datetime.datetime.combine(state.fltDateStart, state.fltTimeStart)
            selEnd = datetime.datetime.combine(state.fltDateEnd, state.fltTimeEnd)

            expanderFltDate.warning('O intervalo máximo é de 1 semana para um melhor desempenho')
            ...
            if state.fltTags != []:
                ...
                st.markdown("------------------------------------------")
                st.markdown("Perfeito, agora é só selecionar as opções abaixo para iniciar a exploração dos dados.")
                ...
                state.execCleaning = st.checkbox(label='Fazer limpeza Automática dos dados', value= (state.execCleaning or False), key='execCleaning')
                if (state.execCleaning):
                    ...
                    if selectCleaning == "Drop NaN":
                        ...
                        methodInterpolation = ["linear", "nearest", "zero", "slinear", "quadratic", "cubic"]
                        ...
                        dfCleaned = dfRawRange.interpolate(method=selectInterpolation, inplace=False)
                ...
                if (showInfoVar):
                    ...
                    if state.dfRawRange[fltPlot].isnull().sum() > 0:
                        ...
                        st.write("Análise de decomposição da série temporal (tendência, sazonalidade e ruído)")
                    ...
                    figHist = px.histogram(state.dfRawRange, x=fltPlot, marginal="box")
