...
def getBestLags(df, tags, target, shiftFrom, shiftTo, shiftStep):
    ...
    for tag in tags:
        ...
        for lag in range(shiftFrom, shiftTo, shiftStep):
            ...
            tempDf = pd.concat(
                [df[tag].shift(lag, freq='min'), df[target]], axis=1)
        ...
        resultPearson[tag] = pd.DataFrame.from_dict(
            vetorPearson, orient='index', columns=[tag])
        resultMic[tag] = pd.DataFrame.from_dict(
            vetorMic, orient='index', columns=[tag])
        resultSpearman[tag] = pd.DataFrame.from_dict(
            vetorSpearman, orient='index', columns=[tag])
        resultKendall[tag] = pd.DataFrame.from_dict(
            vetorKendall, orient='index', columns=[tag])
...
def write(state):
    ...
    if type(state.dfRawRange) == type(None):
        ...
        if type(state.fltTarget) == type(None):
            st.warning(
                '**Atenção** - Antes de iniciar essa etapa, navegue pela aba **Data Correlation** e **selecione** uma variável alvo no passo 1')
            ...
            if len(state.fltTags) > 0:
                ...
                dfRawCorr = calcCorr(
                    state.dfRawRange, state.fltTags, state.fltTarget)
                st.dataframe(dfRawCorr)

                st.info(
                    "Os resultados do cálculo de correlação são a base para o cálculo de defasagem (lag) que iremos realizar no Passo 2.")
                st.subheader('')

                st.markdown(
                    '2. Vamos agora utilizar o algoritmo de varredura para encontrar o melhor valor de defasagem entre a(s) variável(is) de processo e qualidade.')
                st.markdown(
                    'Entretanto, o algoritmo de varredura precisa de saber o **tamanho da janela de varredura**. Nesse caso, selecione o tamanho da janela (escala em minutos):')
                st.markdown('')

                parSearchLimits = st.slider(
                    label='Selecione a janela de busca (minutos)', min_value=0, max_value=360, value=(0, 360), step=5)
                ...
                st.warning(
                    "**Nota**: quanto maior a janela de varredura, maior o tempo de execução dos cálculos.")
                ...
                if btnBestLag:
                    ...
                    state.maximumPearson, state.maximumidxPearson = getMaxCorr(
                        state.dicRawBestLagPearson, fltTags)
                    state.maximumSpearman, state.maximumidxSpearman = getMaxCorr(
                        state.dicRawBestLagSpearman, fltTags)
                    state.maximumKendall, state.maximumidxKendall = getMaxCorr(
                        state.dicRawBestLagKendall, fltTags)
                    state.maximumMic, state.maximumidxMic = getMaxCorr(
                        state.dicRawBestLagMic, fltTags)

                    st.success("Finalizado!")

                fltRawBestLagCurve = st.selectbox(
                    label='Process Variable:', options=fltTags, key='fltRawBestLagCurve')
                if len(fltRawBestLagCurve) > 0:
                    ...
                    if (state.dicRawBestLagPearson != None) & (state.dicRawBestLagSpearman != None) & (state.dicRawBestLagKendall != None) & (state.dicRawBestLagMic != None):
                        ...
                        annotations.plot(
                            state.dicRawBestLagPearson[fltRawBestLagCurve], lw=1, color='r', label="Pearson")
                        annotations.scatter(
                            x=state.maximumidxPearson[indice_variavel], y=state.maximumPearson[indice_variavel], color="red")

                        annotations.plot(
                            state.dicRawBestLagSpearman[fltRawBestLagCurve], lw=1, color='orange', label="Spearman")
                        annotations.scatter(
                            x=state.maximumidxSpearman[indice_variavel], y=state.maximumSpearman[indice_variavel], color="orange")

                        annotations.plot(
                            state.dicRawBestLagKendall[fltRawBestLagCurve], lw=1, color='k', label="Kendall")
                        annotations.scatter(
                            x=state.maximumidxKendall[indice_variavel], y=state.maximumKendall[indice_variavel], color="k")
                        ...
                        annotations.set_title('Métodos de Correlação na Janela de Varredura \n {} x {}'.format(
                            state.fltTarget, fltRawBestLagCurve))
                        ...
                        independent_variables = set(
                            state.fltTags) - set(dependent_variable)
                        ...
                        for lag in list_lags:
                            counter = 0
                            if(lag > 0):
                                gc_res[lag] = grangercausalitytests(
                                    state.dfRawRange[list_columns], [lag])
                            else:
                                gc_res[lag] = None
                            ...
                            if(len(list_methods) > 1):
                                ...
                                if(counter > 0):
                                    st.markdown(
                                        "Resultado dos **testes de hipótese** para o **best lag** encontrado pelos métodos **" + methods + "**:")
                                    st.info("Valores passados (" + str(lag) + " minutos) de " + list_columns[1] + " tem um efeito estatisticamente significante nos valores atuais de " + list_columns[0] + ". Passou em " + str(
                                        np.round(((counter/4)*100), 2)) + " % dos testes de hipótese.")
                                else:
                                    st.markdown(
                                        "Resultado dos **testes de hipótese** para o **best lag** encontrado pelos métodos " + methods + ":")
                                    st.info("Valores passados (" + str(lag) + " minutos) de " + list_columns[1] + " não tem um efeito estatisticamente significante nos valores atuais de " + list_columns[0] + ". Passou em " + str(
                                        np.round(((counter/4)*100), 2)) + " % dos testes de hipótese.")

                            else:
                                if(counter > 0):
                                    st.markdown(
                                        "Resultado dos **testes de hipótese** para o **best lag** encontrado pelo método **" + k_v_exchanged[lag][0] + "**:")
                                    st.info("Valores passados (" + str(lag) + " minutos) de " + list_columns[1] + " tem um efeito estatisticamente significante nos valores atuais de " + list_columns[0] + ". Passou em " + str(
                                        np.round(((counter/4)*100), 2)) + " % dos testes de hipótese.")
                                else:
                                    st.markdown(
            ...
            if ((len(state.fltTags) > 0)):
                ...
                if st.checkbox("Clique aqui para ver os datasets sincronizados"):
                    ...
                    fltMethodLag = st.selectbox(
                        label='Metódo do lag: ', options=methodLag, key='fltMethodLag')
                    ...
                    for tag in fltTags:
                        ...
                        if fltMethodLag == "Pearson":
                            if np.isnan(state.maximumidxPearson[indexTag]):
                                ...
                                state.dfRawRangeShifted[tag] = state.dfRawRangeShifted[tag].shift(
                                    0, freq='min')
                            else:
                                state.dfRawRangeShifted[tag] = state.dfRawRangeShifted[tag].shift(
                                    state.maximumidxPearson[indexTag], freq='min')
                            ...
                            if np.isnan(state.maximumidxSpearman[indexTag]):
                                ...
                                state.dfRawRangeShifted[tag] = state.dfRawRangeShifted[tag].shift(
                                    state.maximumidxSpearman[indexTag], freq='min')
                            ...
                            if np.isnan(state.maximumidxKendall[indexTag]):
                                ...
                                state.dfRawRangeShifted[tag] = state.dfRawRangeShifted[tag].shift(
                                    state.maximumidxKendall[indexTag], freq='min')

                        elif fltMethodLag == "Mic":
                            state.dfRawRangeShifted[tag] = state.dfRawRangeShifted[tag].shift(
                                state.maximumidxMic[indexTag], freq='min')
                    ...
                    st.write(
                        "**Pronto!** Curtiu? Caso o Pandorabox tenha te ajudado de alguma forma, escreve pra gente e conta como foi ou se tá precisando de mais alguma coisa!")
                    st.write(
                        "Nosso e-mail: **inteligenciaindustrial@ihm.com.br**")
