...
def heatmapMIC(df, mine):
    ...
    heatmap = sns.heatmap(corrMic, vmin=0, vmax=1,
                          mask=maskCorr, annot=True, cmap="Reds")
...
def heatmapPPS(df):
    ...
    matrix_df = ppscore.matrix(df, random_seed=123)[['x', 'y', 'ppscore']].pivot(
        columns='x', index='y', values='ppscore')

    fig, heatmap = plt.subplots()
    heatmap = sns.heatmap(np.round(matrix_df, 2), vmin=0,
                          vmax=1, cmap="Blues", annot=True)
    heatmap.set_title("Estimativa do 'Potencial preditivo' do problema", fontdict={
                      'fontsize': 12}, pad=12)
...
def plotComparacaoCorrelacao(df, tag, target):
    ...
    figScatter.suptitle("Comparação entre os métodos de correlação".format(
        tag, target), fontsize="large")
    ...
    sns.barplot(x=methodScatterCorr, y=calcScatterCorr,
...
def write(state):
    ...
    st.markdown(
        'Após prepararmos os dados, vamos agora analisar os **cálculos de correlação** (etapa anterior a de sincronização).')
    ...
    if type(state.dfRawRange) == type(None):
        ...
        expanderVarCorr = st.sidebar.expander(
            "Passo 1: Selecione a variável", expanded=False)

        state.fltTarget = expanderVarCorr.selectbox(
            label='Variável alvo do cálculo de correlação:',
            options=state.fltTags,
            index=state.fltTags.index(
                state.fltTarget) if state.fltTarget else 0,
        ...
        # Correlação simples
        expanderMethodCorr = st.sidebar.expander(
            "Passo 2: Selecione o método para a correlação", expanded=False)
        optionCorr = ["pearson", "kendall", "spearman"]
        state.methodCorr = expanderMethodCorr.selectbox(
            label='Método de cáclulo da Correlação', options=optionCorr, index=optionCorr.index(state.methodCorr) if state.methodCorr else 0)

        if len(state.fltTags) > 0:
            st.markdown(
                'Selecione as opções abaixo **(uma por vez)** para avaliar os dados:')

            showRawTimeSeries = st.checkbox(
                label='Visualizar as séries temporais das variáveis', value=False, key='showRawTimeSeries')
            if (showRawTimeSeries):
                fltSelectedVar = st.multiselect(
                    label='Escolha a variável de processo que deseja visualizar:', options=state.fltTags, key='fltSelectedVar')
                if (len(fltSelectedVar) > 0):
                    st.line_chart(
                        data=state.dfRawRange[fltSelectedVar], width=1000, height=150, use_container_width=True)
                    st.line_chart(
                        data=state.dfRawRange[state.fltTarget], width=1000, height=150, use_container_width=True)

            if len(state.fltTags) < 2:
                st.warning(
                    "Selecione no mínimo duas variaveis no passo 1 do **'Data Preparation'** para análise.")

            # Realiza a correlação do dataframe filtrado pelas variaveis selecionadas e intervalo de tempo
            showRawCorr = st.checkbox(
                label='Visualizar as correlações lineares', value=False, key='showRawCorr')
            if (showRawCorr) & (len(state.fltTags) >= 2):
                ...
                maskCorr = np.triu(np.ones_like(state.dfRawRange.corr(
                    method=state.methodCorr), dtype=np.bool))

                figCorr, heatmapCorr = plt.subplots()
                heatmapCorr = sns.heatmap(np.round(state.dfRawRange.corr(
                    method=state.methodCorr), 2), vmin=-1, vmax=1, mask=maskCorr, annot=True, cmap="coolwarm")
                heatmapCorr.set_title("Correlação de {}".format(
                    state.methodCorr), fontdict={'fontsize': 12}, pad=12)
                st.pyplot(figCorr)
                ...
                if (state.methodCorr == "pearson"):
                    ...
                    showLinearCorr = st.checkbox(
                        label='Visualizar análises complementares à de correlação linear de pearson', value=False, key='showLinearCorr')
                    if (showLinearCorr == True):
                        #st.title("Análises complementares de Rˆ2")

                        varAnalise = st.selectbox(
                            label='Primeiro, selecione a variável para realizarmos as análises complementares:', options=state.fltTags, key='varAnalise')
                        ...
                        if not state.dfRawRange[state.fltTarget].isnull().values.any():
                            regLinear = linregress(
                                state.dfRawRange[varAnalise], state.dfRawRange[state.fltTarget])
                            resultRegLinear = regLinear.intercept + \
                                regLinear.slope*state.dfRawRange[varAnalise]

                            figRegLinear = plt.figure()
                            plt.plot(
                                state.dfRawRange[varAnalise], state.dfRawRange[state.fltTarget], 'o', alpha=0.1, markersize=4)
                            plt.plot(
                                state.dfRawRange[varAnalise], resultRegLinear, alpha=0.6)
                            plt.xlabel('{}'.format(varAnalise))
                            plt.ylabel('{}'.format(state.fltTarget))
                            plt.title('Scatter plot of {} versus {}'.format(
                                varAnalise, state.fltTarget))
                            ...
                            resid = state.dfRawRange[state.fltTarget] - \
                                resultRegLinear
                            a = np.corrcoef(
                                state.dfRawRange[varAnalise], state.dfRawRange[state.fltTarget])
                            rho_actual = a[0, 1]
                            R2 = 1 - resid.var() / \
                                state.dfRawRange[state.fltTarget].var()

                            st.subheader(
                                "Análise complementar 1 - Resíduos (por meio de regressão linear)")
                            ...
                            st.subheader(
                                "Análise complementar 2 - Erro Médio Absoluto")

                            MAE_after = np.abs(resid).std()
                            deviation = state.dfRawRange[state.fltTarget] - \
                                state.dfRawRange[state.fltTarget].mean()

                            MAE_before = np.abs(deviation).std()
                            improvementMAE = 1 - MAE_after / MAE_before
                            improvementMAE_percent = (
                                1 - MAE_after / MAE_before)*100
                            ...
                            # brenão, estava varAnalise, mas aqui é o target!
                            RMSE_before = state.dfRawRange[state.fltTarget].std(
                            )
                            improvementRMSE = 1 - RMSE_after / RMSE_before
                            improvementRMSE_percent = (
                                1 - RMSE_after / RMSE_before)*100
                        ...
                        #             \n\nUsing MAPE implies that an error of 1 IQ point is more important for someone with low IQ and less important for someone with high IQ. In this context, it's not clear whether that's true.")
                            ...
                            st.warning(
                                "A variável em análise possui valores **NaN**, realize a limpeza em **Data Preparation**.")
            ...
            showStatisticalSignificance = st.checkbox(
                label='Visualizar Teste Estatístico de Significância', value=False, key='showStatisticalSignificance')
            if (showStatisticalSignificance):
                fltSelectedVarStatisPreditora = st.multiselect(
                    label='Escolha a(s) variável(s) independente(s):', options=state.fltTags, key='fltSelectedVarStatisPreditora')
                if (len(fltSelectedVarStatisPreditora) > 0):
                    list_independent_variables = []
                    for i in range(0, len(fltSelectedVarStatisPreditora)):
                        list_independent_variables.append(
                            fltSelectedVarStatisPreditora[i])
                    X2 = sm.add_constant(
                        state.dfRawRange[list_independent_variables])
                    ...
                    for i in range(0, len(fltSelectedVarStatisPreditora)):
                        if(p_valor_variavel[i] < 0.05):
                            st.markdown(
                                "A variável " + str(fltSelectedVarStatisPreditora[i]) + " é estatisticamente significante para a previsão de " + str(state.fltTarget))
                            ...
                            st.markdown(
                                "A variável " + str(fltSelectedVarStatisPreditora[i]) + " não é estatisticamente significante para a previsão de " + str(state.fltTarget))
                            st.markdown(
                                "There is insufficient evidence in your sample to conclude that a non-zero correlation exists")
